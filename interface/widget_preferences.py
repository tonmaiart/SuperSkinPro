...
def _draw_preferences(layout, context, prefs):
    ...
    (formerly the standalone CUSTOMIZE tab) are all gone: "Single Mode
    Color Ramp" moved into the `vgcolor` feature domain (see
    features/vgcolor/README.md), which now wraps Blender's own native
    weight-paint color ramp instead of this addon's custom draw-shader
    ramp; the other two were removed outright (no longer necessary now
    that the custom shader is gone) rather than migrated. "Multi Color
    Preview" (the `multi_color_preview` domain's own PREFERENCE-tab
    section, which only ever rendered an empty box -- draw_section() is a
    no-op, all real interaction is the Alt+3 hold gesture) is likewise
    gone: that domain now registers with `draw_tab=""` instead of being
    excluded here, since there was never anything in its box to exclude a
    double-draw of.
    ...
    row.operator("superskin.reset_prefs", text="Reset to Default", icon='LOOP_BACK')
    row.operator("superskin.reset_license_activation", text="Reset All Activate", icon='TRASH')
