...
What remains here:
  ...
  - X-ray warning HUD text draw callback
...
# ═══════════════════════════════════════════════════════════════════════════
#  X-ray HUD text callback (registered permanently at addon startup)
# ═══════════════════════════════════════════════════════════════════════════

_hud_text_handle = None


def _draw_text_2d_callback():
    context = bpy.context
    if not context.space_data or context.space_data.type != 'VIEW_3D':
        return
    ...
    xray_text = "X-RAY TURN ON (COLOR VISUALIZER OVERRIDE)"
    text_w, _ = blf.dimensions(font_id, xray_text)
    center_x = max(0, region_width // 2 - int(text_w) // 2)
    blf.position(font_id, center_x + 1, y_offset + 1, 0)
    blf.color(font_id, 0.0, 0.0, 0.0, 0.9)
    blf.draw(font_id, xray_text)
    blf.position(font_id, center_x, y_offset, 0)
    blf.color(font_id, 1.0, 0.4, 0.0, 1.0)
    blf.draw(font_id, xray_text)
...
def register():
    global _hud_text_handle
    _hud_text_handle = bpy.types.SpaceView3D.draw_handler_add(
        _draw_text_2d_callback, (), 'WINDOW', 'POST_PIXEL'
    )


def unregister():
    global _hud_text_handle
    if _hud_text_handle is not None:
        try:
            bpy.types.SpaceView3D.draw_handler_remove(_hud_text_handle, 'WINDOW')
        except Exception:
            pass
        _hud_text_handle = None
