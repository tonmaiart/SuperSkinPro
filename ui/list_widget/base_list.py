...
class SuperSkinListMixin:
    ...
    def get_row_operator_id(self, item=None) -> str:
        ...
        *item* is the row's collection element, passed so a single UIList
        can route different row *kinds* within the same domain to
        different operators (e.g. the Bones list's real-vertex-group rows
        vs. its orphaned-weight pseudo-rows, both drawn from the same
        mirror collection — see ``widget_deform_bones.py``). Optional:
        domains with only one row kind (e.g. Layers) can ignore it.
        """
    ...
    def set_row_operator_props(self, op, item):
        ...
        Subclasses MUST override this.  *op* is the operator instance
        returned by ``layout.operator()``; *item* is the collection
        element being drawn.
        """
