...
The adapter *protocol* is still useful for the Bones domain (where
Ctrl/Shift/Alt+Shift modifier-key behaviour is non-trivial), but it is
**no longer used by a shared operator class** — each domain now has its
own dedicated, synchronous operator with a distinct ``bl_idname``.
...
operator identities across domains.  It can be called from any domain's
operator ``invoke()`` and has no side effects.
"""
...
class ListSelectionAdapter:
    ...
    def get_keys_in_visual_order(self, context, obj) -> list:
        ...
        Used by range-select (Shift-click) and select-all-visible
        (Alt+Shift-click) to pick the correct span of rows.
        """
    ...
    def on_single_select(self, context, obj, key: str):
        ...
        Bones: persist selection to active layer, set active bone name.
        Layers: switch to the clicked layer, enforce visualizer, re-apply
        active bone, sync the UI collection.
        """
        raise NotImplementedError
...
def register_adapter(domain: str, adapter: ListSelectionAdapter):
    ...
    Called at module-load / ``register()`` time by the owning UI file.
    """
...
# ==============================================================================
# Pure modifier-key selection logic (no bpy side effects, no operator coupling)
# ==============================================================================
...
def resolve_row_click_selection(selected_keys, last_key, history,
    ...
    Args:
        ...
        event: ``bpy.types.Event`` — the Blender event, read for
               ``event.alt``, ``event.shift``, ``event.ctrl``.

    Returns:
        ``(selected_keys: set[str], last_key: str | None, history: list[str])``
        — the new state **after** resolving the click.
    """
    ...
    # ── Shift: range-select ─────────────────────────────────────────────
        ...
        if item_key in visual_order and last_key in visual_order:
            ...
            # shift-click with no valid anchor — treat as plain click
            selected_keys.clear()
...
def _build_range_history(prev_history, visual_order, start_pos, end_pos):
    """Return a new history list reflecting a range-select between
    *start_pos* and *end_pos* in *visual_order*."""
