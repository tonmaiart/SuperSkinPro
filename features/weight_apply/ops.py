...
# Drag preview is snapped to a coarse 0.05 grid by default; holding Ctrl
# zooms into a fine 0.001 sub-grid within a small window around wherever the
# drag currently is, for precise per-vertex/joint adjustments. A background
# thread speculatively pre-computes neighboring grid buckets (biased toward
# the observed drag direction, or toward the fine sub-grid window while Ctrl
# is held) so crossing a bucket boundary is normally a cache hit -- a direct
# `_finish_write()` call, no Rust FFI wait -- rather than a fresh compute.
...
class SUPERSKIN_OT_weight_gesture(bpy.types.Operator):
    ...
    drag continuously: a coarse 0.05 step by default, or a fine 0.001 step
    within a small window around wherever the drag currently is while Ctrl
    is held (`_quantize()`, `_GRID_COARSE_STEP`/`_GRID_FINE_STEP`). Ctrl
    ...
    def _apply_result(self, context, real_action, action_data, intensity, result):
        ...
        if write_result.get("status") != "CANCELLED":
            if self._fine_mode:
                anchor_value = self._fine_anchor_idx / _grid_n(False)
                mode_suffix = f" [Fine ±{_GRID_FINE_HALF_RANGE:.2f} @{anchor_value:.2f}]"
    ...
    def modal(self, context, event):
        if event.type == 'MOUSEMOVE':
            ...
            if self._is_dragging:
                ...
                fine_anchor_value = (
                    self._fine_anchor_idx / _grid_n(False) if self._fine_mode else None
                )
                ...
                draw.update(
                    ...
                    fine_mode=self._fine_mode, fine_anchor=fine_anchor_value,
                )
