...
Sync is bidirectional:
  ...
  - Picking an object in the viewport is detected at the top of
    ...
    one-shot ``bpy.app.timers`` callback -- the same pattern
    ``interface/utils/utils.py::_auto_init_layers`` uses, since Blender
    forbids some ID-related mutations from inside a draw() call and timers
    are the established escape hatch in this codebase.
