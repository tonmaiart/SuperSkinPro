...
active:
  ...
  - `context.preferences.view.weight_color_range` (a `ColorRamp`) — set to
    this domain's `edit_ramp` or `mask_ramp` stops (whichever applies).
...
This domain's own two ramps (`edit_ramp` / `mask_ramp`) live on hidden,
addon-owned Texture datablocks so the settings UI can use Blender's native
ColorRamp widget — see `_ramp_io.py`. This module only reads their current
stops; it never edits them.
...
def _get_ramp_stops(ramp_id: str) -> list:
    """Read + sort a ramp's stops straight off this domain's own hidden
    ramp texture (see ``_ramp_io.py``)."""
    try:
        tex = _ramp_io.get_or_create_ramp_texture(ramp_id)
        return _ramp_io.read_stops(tex.color_ramp)
    except Exception:
        return []
