...
active color attribute, and ``overlay.show_weight`` -- see below) is
snapshotted the moment the preview starts and restored exactly on stop --
the same pattern used by ``native_sync.py`` for the weight-color ramps.
...
def _recompute_and_write(obj, force: bool = False) -> None:
    ...
    key = _make_color_key(obj, bm, frame)
