...
see `overlay_color_feature.py`'s module docstring for why. Owns:
  ...
  - Two customizable ramps living on hidden Texture datablocks so the
    settings UI can use Blender's native ColorRamp widget
    (`_ramp_io.py` + `overlay_color_feature.py`).
