...
# addon root. The vendored path logic below (backup/restore/reinstall,
# updater status JSON location) assumes a root-level location, so this
# constant preserves that assumption after the move.
...
def _rmtree_safe(path):
    ...
    ``shutil.rmtree`` raises ``PermissionError`` whenever ``path`` (backup
    copies, the staging folder, or the addon root itself) contains a ``.git``
    directory checked out on this platform -- e.g. when the addon is
    installed via a local git clone rather than a packaged zip.
    """
...
class SingletonUpdater:
    ...
    def __init__(self):
        ...
        # since backup/restore/reinstall logic below and the updater's own
        # status JSON filename must all resolve to the actual installed addon
        # location, not features/addon_updater/.
    ...
    def create_backup(self):
        ...
        CoreFacade.debug_log("feature_domains", "addon_updater.create_backup(): entry")
        ...
        if self._backup_ignore_patterns is not None:
            try:
                ...
                CoreFacade.debug_log(
                    "feature_domains",
                    "addon_updater.create_backup(): FAILED to create backup (ignore-patterns path), continuing update anyway")
            ...
            try:
                ...
                CoreFacade.debug_log(
                    "feature_domains",
                    "addon_updater.create_backup(): FAILED to create backup, continuing update anyway")
                print("Failed to create backup, still attempting update.")
                self.print_trace()
                return
        ...
        CoreFacade.debug_log("feature_domains", "addon_updater.create_backup(): backup complete")
    ...
    def restore_backup(self):
        ...
        CoreFacade.debug_log("feature_domains", "addon_updater.restore_backup(): entry")
        ...
        self.save_updater_json()
        CoreFacade.debug_log("feature_domains", "addon_updater.restore_backup(): restore complete, reloading addon")
...
def read_repo_config():
    ...
    token, backup/overwrite patterns, branch list, etc.) -- edit that JSON
    file directly instead of this module. Falls back to hardcoded safe
...
def configure(skip_tag=None, select_link=None):
    ...
    Updater.backup_current = cfg.get("backup_current", True)

    # Sample ignore patterns for when creating backup of current during update.
    Updater.backup_ignore_patterns = cfg.get("backup_ignore_patterns", ["__pycache__"])
