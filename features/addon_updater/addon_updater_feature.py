...
def register():
    ...
    # widget_preferences.py's _auto_init_layers / debug_console's
    # _tick_redraw) keeps register() itself minimal while still checking on
    # every addon-enable and F3 Reload Scripts, without a manual "check now"
    # button anywhere. The check itself is a no-op unless the configured
    # interval (default: every 7 days, see default_config.json) has
    # actually elapsed -- see engine.py's past_interval_timestamp().
