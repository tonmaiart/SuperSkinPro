...
no_influence_color / static_wedge_width).  Hardcoded tuples below serve as
fallback only.

A deform bone with zero weight anywhere on the active layer draws in
``no_influence_color`` (gray by default) instead of ``static_default_color``,
...
the "default" (unselected, unhovered) state -- active/hover/multi-select
colors still take priority regardless of influence.
...
_COLOR_NO_INFLUENCE = (0.5, 0.5, 0.5, 0.35)


def _get_static_prefs():
    """Return (active_color, multi_color, default_color, no_influence_color,
    hover_color, wedge_width, line_width, hover_extra, hold_extra,
    overall_size, pivot_ratio, fill_opacity, head_circle_size)."""
    try:
        ...
        return (_COLOR_ACTIVE, _COLOR_MULTI, _COLOR_DEFAULT, _COLOR_NO_INFLUENCE,
                (1.0, 0.55, 0.0, 1.0), _HEAD_HALF_WIDTH, 1, 2, 3, 1.0, 0.333, 0.25, 0.55)
...
def _draw_callback():
    ...
    # weights here. Defensively guarded, same as _get_static_prefs() below --
    ...
    (color_active, color_multi, color_default, color_no_influence, color_hover,
