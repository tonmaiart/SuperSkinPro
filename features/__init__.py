...
The viewer domains must be first so they render at the top of their tabs:
  ...
  SKINNING tab: deform_bone_viewer (first, non-collapsible) → weight_apply → mirror → clipboard → auto_block → circle_tool_adjust → in_mesh_transfer
  PREFERENCE  : bone_picker → multi_color_preview → vgcolor → debug_console (hosted under SYSTEM tab)
...
from . import vgcolor              # PREFERENCE tab -- wraps Blender's native weight-paint color ramp
