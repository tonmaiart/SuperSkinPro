...
#
# Does NOT build or commit anything itself -- it only force-moves a tag
# named after blender_manifest.toml's `version` field to the current tip of
# `main` and pushes it to `origin` (the dev repo), after verifying `main` is
# clean and fully in sync with `origin/main`. Pushing that tag is what
# .github/workflows/release.yml's `on: push: tags: ["[0-9]+.[0-9]+.[0-9]+"]`
# trigger is watching for; the actual cross-platform Rust build + dev-file
# cleanup + force-push of the cleaned result (as `main`, tagged with the
# same version) to https://github.com/tonmaiart/SuperSkinPro.git all happen
# there, on GitHub's runners, not on this machine. There is no more
# `release` branch anywhere in this pipeline.
set -euo pipefail
